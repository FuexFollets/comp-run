...
from style_strings import bquoted
...
class Sort_args:
    def __init__(self, argv: list[str]):
        ...
        if not os.path.exists(self.path):
            sys.stderr.write(f"{style_strings.comprun_name} {style_strings.error_label} unable to find file at path {bquoted(self.path)}")
            raise RuntimeError("Invalid file path")

        self.options = {*self.options, *unsplit_options.split('-')}

        try:
            self.compiler_name = next(arg_values)
        except:
            sys.stderr.write(f"{style_strings.comprun_name} {style_strings.error_label} compiler name is nonexistent")
    ...
    def compile_program(self) -> None:
        try:
            ...
            sys.stderr.write(f"{style_strings.comprun_name} {style_strings.error_label} unrecognized compiler {bquoted(self.compiler_name)}\n")
